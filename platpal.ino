...
String returnHeader()
{
  ...
  ptr += "<title>Plant Pal V0.2</title>\n";
  ...
  ptr += "</head>\n";
  ptr += "<body>\n";
  ptr += "<header><h1>Plant Pal V0.2</h1></header>\n";
...
void handleJS() {
  ...
  // Add code to retain only the last 8 readings
  ptr += "    if (chartData.length >= 8) {\n";
  ptr += "      chartData.shift();\n"; // Remove the oldest reading
  ptr += "    }\n";

  ptr += "    var currentTime = new Date().toLocaleTimeString();\n"; // Get the current timestamp
  ptr += "    chartData.push({\n"; // Push the new data to the array
  ptr += "      time: currentTime,\n"; // Store the current timestamp
  ptr += "      temp: data.temp,\n";
  ptr += "      hum: data.hum,\n";
  ...
  // Extract time data from the array
  ptr += "  var timeData = chartData.map(function(entry) {\n";
  ptr += "    return entry.time;\n";
  ptr += "  });\n";
  ...
  ptr += "    type: 'line',\n";
  ptr += "    data: {\n";
  ptr += "      labels: timeData,\n"; // Use the extracted time data for labels
  ptr += "      datasets: [\n";
  ptr += "        {\n";
  ptr += "          label: 'Temperature',\n";
  ptr += "          data: chartData.map(function(entry) { return entry.temp; }),\n"; // Use the extracted temperature data
  ...
  ptr += "        {\n";
  ptr += "          label: 'Humidity',\n";
  ptr += "          data: chartData.map(function(entry) { return entry.hum; }),\n"; // Use the extracted humidity data
  ...
  ptr += "        {\n";
  ptr += "          label: 'Moisture',\n";
  ptr += "          data: chartData.map(function(entry) { return entry.moisture; }),\n"; // Use the extracted moisture data
  ...
  ptr += "setInterval(timer, 3000);\n";

  ptr += "function getRandomFact()\n";
  ptr += "{\n";
  ptr += "$.getJSON(\"/randomfact.json\", function(data) {\n";


  ptr += "console.log(data);\n";
  ptr += "$(\"#randomFact\").text(data.randomfact);\n";
    
  ptr += "});\n";
   
  ptr += "}\n";

  ptr += "setInterval(getRandomFact, 10000);\n";
